...
`/` — or tier letter), `--element-version`, `--data-dir`, `--lang`, `--format text|json`. For editor
integration, `--stdin --filename NAME` checks a single buffer read from stdin (per-file rules only);
the JSON payload (`{diagnostics, summary}`) is the same one the MCP server returns.
