...
# (docs: "Типы, порождаемые элементом проекта"). A suffix of another kind passes as a
# false negative; the union keeps the rule free of false positives.
...
# object's tabular sections and module-declared types. Other kinds are skipped.
    ...
    for s in sources:
        ...
        for start in _type_ref_starts(toks):
            ...
            for chain in chains:
                ...
                if rec is None or rec["kind"] not in _CHECKED_KINDS:
                    continue
                seg = chain[1]
                if seg.value in _MEMBER_TYPE_TAILS or seg.value in rec["members"]:
                    continue
                diags.append(Diagnostic(
