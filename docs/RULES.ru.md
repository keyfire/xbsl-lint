...
список в рантайме – `xbsl list-rules` (или MCP `list_rules`). Сейчас правил: 87.

## Граница: линтер дополняет компилятор, но не заменяет его

Линтер работает по тексту, AST и модели проекта, зная типы только "на первом шаге"
(объявленные номинальные типы переменных – без вывода типов выражений). Он ловит то, что
компилятор Элемента не проверяет или сообщает невнятно (соглашения, типографику, структуру,
ссылки на несуществующие типы и объекты), плюс доказуемые ошибки уровня сигнатур (возврат
не по сигнатуре метода, неверное число аргументов локального вызова, не-исключение в
`поймать`, отсутствующий член простого stdlib-типа), но НЕ ловит того, что требует полного
вывода: избыточное приведение, незакрытый ресурс, тип возвращаемого значения. Проверка
корректности кода – серверная компиляция при деплое; линтер идёт перед ней и снимает
